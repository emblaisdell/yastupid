...
Writing $g=|a+b-c|$ and $M=\max(a+b,c)+1$, we prove that for
\emph{every} false sum $(a,b,c)$ with $a,b,c\ge 1$ and all
$s,t\ge M$, the single ball $s$ can be transformed into the single ball $t$
if and only if $g \mid t-s$; moreover the threshold $M$ cannot be lowered
to $M-1$ in general.  The sufficiency proof is a taxonomy: a generic
...
Let $a,b,c\ge 1$, let $g=|a+b-c|$ and $M=\max(a+b,c)+1$.  For all
$s,t\ge M$,
\[
\ms{s} \reach \ms{t}
...
$t=s$.)  Moreover, the threshold $M$ is optimal: it cannot be lowered to
$M-1$.  For the Classic configuration $9+10=21$ one has $M=22$, the pair
$(21,23)=(M-1,M+1)$ satisfies the congruence, and yet
$\ms{21}\not\reach\ms{23}$.
\end{theorem}
...
Set $\delta=a+b-c\in\Z$, \ $g=|\delta|$, and $M=\max(a+b,c)+1$.
\end{definition}

\begin{example}
Classic ($9+10=21$): $g=2$, $M=22$.  The false split lowers the
...
For $\lie{9}{10}{21}$ one has $M=22$: \ $\ms{21}\not\reach\ms{23}$, although
$21,23\ge M-1=21$ and $2\mid 23-21$.
\end{proposition}
...
Thus the guarantee of Theorem~\ref{thm:main-intro} fails at $M-1=\max(a+b,c)$
itself, so $M$ is the optimal uniform threshold expressible in terms of
$\max(a+b,c)$.  (For particular false sums a smaller threshold may hold; we do
not pursue the per-configuration optimum here.)
...
Theorem~\ref{thm:main} with $M=\max_i\max(a_i+b_i,c_i)+1$.
...
\item \textbf{Per-configuration thresholds.}  $M=\max(a+b,c)+1$ is optimal as
a uniform function of $\max(a+b,c)$ (Proposition~\ref{prop:sharp}), but individual
